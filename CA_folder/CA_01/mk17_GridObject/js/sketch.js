...
function setup() {
  ...
  let padding = -moduleSize / 4;
  //set the padding around the grid between it and the canvas edges
  let canvasPadding = (moduleSize + padding) * 2;
  ...
  //pass the variables into the grid and generate a new grid object
  //constructor(moduleSize, padding, cols, rows, canvasPadding, moduleStrokeThickness) {
  grid = new Grid(moduleSize, padding, colNum, rowNum, canvasPadding, 1);
...
function keyReleased() {
  ...
  //Save Canvas with S
  else if (key == 's' || key == 'S') saveCanvas(`mk17_W:${width}_H:${height}`)
  //Toggle info with I
  else if (key == 'i' || key == 'I') {
    if (displayInfo) {
      displayInfo = false;
